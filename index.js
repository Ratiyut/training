            ...
            if(response.RespCode == 200) {
                ...
                for (let i = 0; i < product.length; i++) {
                    html += `<div onclick="openProductDetail(${i})" class="product-items ${product[i].type}">
                                ...
                                <p stlye="font-size: 1vw;">${ product[i].ID_product }</p>
                            </div>`;
                }
...
function openProductDetail(index) {
    ...
    $("#mdd-price").text( (product[index].ID_product))
            ...
            html += `<div class="cartlist-items">
                        <div class="cartlist-left">
                            ...
                            <div class="cartlist-detail">
                                ...
                                <p style="font-size: 1.2vw;">Product Number: ${cart[i].id}</p>
                            </div>
                        </div>
        ...
        success: function (response) {
            ...
            if (response.RespCode == 200) {
                Swal.fire({
                    icon: 'success',
                    title: 'Thank you',
                    html: ` <p> Number of Products: ${response.Amount.ProductCount}</p>
                            <p> Shipping : ${response.Amount.Shipping}</p>
                            <p> Vat : ${response.Amount.Vat}</p>
                            <p> Netamount : ${response.Amount.ProductCount}</p>
                            `
                }).then((res) => {
                    if (res.isConfirmed) {
                        cart = [];
